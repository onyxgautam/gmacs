...
(autoload 'og/dired-open-xdg "dired-open")

(defun og/dired-open-xdg (&optional filename)
  ...
  (if (executable-find "xdg-open")
      (let ((file (if filename filename (ignore-errors (dired-get-file-for-visit)))))
        (if (string-match (concat "jp?g\\|png" "$") (file-name-extension file))
            (dired-open--start-process (file-truename file) "pqiv .")
          (dired-open--start-process (file-truename file) "xdg-open")))
    nil))

(bind-key "O" #'og/dired-open-xdg dired-mode-map)
#+END_SRC
...
(defun og/math-preview-toggle-subtree ()
  ...
  (save-restriction
    ...
    (cond (current-prefix-arg (math-preview--clear-region (point-min) (point-max)))
          ((math-preview--overlays (point) (point)) (math-preview-clear-at-point))
          (t (math-preview-all)))))

(use-package math-preview
  :commands (math-preview-at-point math-preview--region math-preview--overlays)
  :custom
  (math-preview-scale 1.1)
  (math-preview-margin '(1 . 1))
  (math-preview-raise 0.325)
  :config
  (add-to-list 'math-preview-tex-marks '("\\begin{align}"  "\\end{align}"))
  (add-to-list 'math-preview-tex-marks '("\\begin{gather}"  "\\end{gather}"))
...
(org-link-set-parameters
 ...
 :export (lambda (path desc format)
           (when (eq format 'latex)
             (format "\\footnote{\\textcolor{red}{\\textbf{TODO:}} %s}" path))))
#+END_SRC
Correction link.
#+BEGIN_SRC emacs-lisp
(org-link-set-parameters
...
(defun og/math-setup ()
  ...
  (setq og/math-snippets (append og/math-global-snippets og/math-local-snippets))
